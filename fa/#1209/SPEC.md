...
- 탭 상단에 안내 문구 표시:
  ...
  - "각 증권사 별 상세 제안 방법은 [가이드북](https://help.betterwealth.co.kr/guidebook/investment/investment-propose/investment-propose-1)을 참고해주세요."
...
- 정보 없음 상태: 계약·미결·발송이력이 모두 없는 그룹은 empty state 카드 표시 ("아직 계약 정보가 없습니다.")
...
- 각 필드의 값 포맷·표기 규칙은 [계약목록.md §6 테이블 컬럼 정의](../../policy/bw-fa/계약목록.md#6-테이블-컬럼-정의) 참조
...
- 사이드모달을 닫았다 다시 열면 이전 선택 상태는 초기화
- 버튼 활성화 조건:
...
- 성공 시
  - 성공 피드백 표시
  - 사이드모달 유지 (닫지 않음) — 다른 포트폴리오를 선택하여 연속 발송 가능
